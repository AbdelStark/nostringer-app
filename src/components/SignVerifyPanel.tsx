...
export default function SignVerifyPanel({
  ...
  return (
    <div className="terminal-console">
      <h2 className="text-xl text-neonBlue glow-text-blue mb-6 border-b border-gray-700 pb-2">Sign & Verify</h2>
      
      <div className="mb-6">
        <label className="block text-sm text-gray-300 mb-2 font-bold">Message to sign:</label>
        <input 
          type="text"
          value={message}
      ...
      {selectedUser && (
        <div className="mb-6 p-3 bg-gray-800 bg-opacity-50 rounded-md border-l-2 border-neonBlue">
          <p className="text-sm text-gray-400">
            Selected signer: <span className="text-neonGreen font-bold glow-text">{selectedUser.label}</span>
            {selectedUser.inRing ? "" : <span className="text-neonYellow glow-text-yellow"> (not in ring)</span>}
          </p>
        </div>
      )}
      ...
      <div className="text-sm whitespace-pre-wrap p-4 bg-black bg-opacity-50 rounded-md border border-gray-700 min-h-32"> 
        {signature && verifyResult === null && (
          <div className="text-gray-300 mb-3 font-mono">
            <span className="text-neonBlue font-bold">$</span> Signature: <span className="text-neonGreen">{formatSig(signature)}</span>
          </div>
        )}
        {verifyResult !== null && (
          <div className="font-mono">
            ...
            {verifyResult ? (
              <span className="text-neonGreen font-bold glow-text crt-flicker">✅ PASSED</span>
            ) : (
              <span className="text-neonRed font-bold glow-text-red crt-flicker">❌ FAILED</span>
            )}
        ...
        {!signature && !verifyResult && (
          <div className="text-gray-500 italic terminal-cursor">Terminal output will appear here...</div>
        )}
      </div>
    </div>
  )
