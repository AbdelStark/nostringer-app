...
export interface UserKey {
  label: string;
  priv: string; // privateKeyHex
  pub: string; // publicKeyHex
  inRing: boolean;
}
          ...
          return (
            <div
              ...
              className={`p-3 rounded-md cursor-pointer transition-colors duration-150
                ${
                  isSelected
                    ? "bg-gray-800 shadow-neon border-l-2 border-neonGreen"
            ...
            >
              <div
                className={`font-bold text-base mb-1 ${
                  isRingMember
                    ? "text-neonBlue glow-text-blue"
                    : "text-neonYellow glow-text-yellow"
                }`}
              >
                {user.label}
                {isRingMember ? "" : " (non-ring)"}
              </div>
              {editingIndex === idx ? (
                <input
                  type="text"
              ...
              <div className="text-gray-400 text-sm mt-1">
                <span className="opacity-80 mr-1">pk:</span>{" "}
                {previewPub(user.pub)}
              </div>
            </div>
          );
